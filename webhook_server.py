...
def get_product_by_media_id(media_id):
    return next((p for p in products if p["media_id"] == media_id), None)
...
def record_plus_user(media_id, user_id):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT dm_sent FROM plus_authors WHERE media_id = %s AND user_id = %s;", (media_id, user_id))
    row = cur.fetchone()
    if row is None:
        cur.execute("""
        ...
        cur.close()
        conn.close()
        return (False, not dm_sent)

def mark_dm_sent(media_id, user_id):
    if not user_id: return
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("UPDATE plus_authors SET dm_sent = TRUE WHERE media_id = %s AND user_id = %s;", (media_id, user_id))
    conn.commit()
    cur.close()
    conn.close()
...
def get_media_insights(media_id, token):
    ...
    r = requests.get(url, params={"metric":"views,reach,saved,shares","access_token":token})
    if r.status_code == 200:
        return {d["name"]: d["values"][0]["value"] for d in r.json().get("data", [])}
    return {}

def get_media_info(media_id, token):
    url = f"{API_BASE}/{API_VERSION}/{media_id}"
    r = requests.get(url, params={"fields":"id,media_type,media_url,timestamp","access_token":token})
    return r.json() if r.status_code == 200 else {}

def refresh_long_lived_token(current_token):
    url = f"{API_BASE}/refresh_access_token"
    r = requests.get(url, params={"grant_type":"ig_refresh_token","access_token":current_token})
    if r.status_code == 200:
        data = r.json()
...
def get_comment_author_id(comment_id, token):
    url = f"{API_BASE}/{API_VERSION}/{comment_id}"
    r = requests.get(url, params={"fields":"from{id,username}","access_token":token})
    if r.status_code == 200:
...
def monitoring():
    ...
    for idx, product in enumerate(products, start=1):
        ...
        if raw_time:
            ...
            except: formatted_time = raw_time
        else:
            formatted_time = "-"
    ...
    try:
        ...
        for change in entry.get("changes", []):
            if change.get("field") == "comments":
                ...
                media_id = media_info.get("original_media_id") if media_info.get("media_product_type")=="AD" else media_info.get("id")

                user_id = (comment_data.get("from") or {}).get("id") or comment_data.get("user_id")
                ...
                if comment_text and set(comment_text) == {"+"} and media_id and comment_id:
                    ...
                    if need_dm:
                        ...
                        if ok and user_id: mark_dm_sent(media_id,user_id)
        ...
        for msg in entry.get("messaging", []):
            if "read" in msg:
                ...
                if mid:
                    ...
                    if media_id: increment_count(media_id,"read_count")
...
def reply_to_comment(comment_id,message,token):
    try:
        requests.post(f"{API_BASE}/{comment_id}/replies",
                      json={"message":message,"access_token":token},timeout=10)
    except Exception as e:
        print("reply_to_comment error:",e)

def send_private_reply(comment_id,product,media_id,token):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if mid: set_mapping(mid,media_id)
            return (True,mid)
        else:
